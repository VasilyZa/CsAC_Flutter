...
class CsacLocalCache {
  ...
  Future<List<ConversationMediaItem>> loadConversationMedia(
    ...
    for (final row in rows) {
      ...
      add(ConversationMediaKind.image, message.emojiAddress);
      add(
  ...
  Future<void> saveMessages(
    ...
    final statement = db.prepare('''
      ...
      ON CONFLICT(conversation_type, conversation_id, id) DO UPDATE SET
        ...
        emoji_address = excluded.emoji_address,
        emoji_abbr = excluded.emoji_abbr,
  ...
  void _migrate(Database db) {
    ...
    db.execute('''
      CREATE TABLE IF NOT EXISTS messages (
        ...
        emoji_address TEXT NOT NULL DEFAULT '',
        emoji_abbr TEXT NOT NULL DEFAULT '',
  ...
  ChatMessage _messageFromRow(Row row) {
    ...
    final emojiAddress = normalizeApiUrl(asString(row['emoji_address']));
    ...
    return ChatMessage(
      ...
      emojiAbbr: asString(row['emoji_abbr']),
